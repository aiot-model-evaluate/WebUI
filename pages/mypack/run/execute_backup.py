...
def exec(choice):
    ...
    # 创建远程目录
    client.exec_command(f"mkdir -p /home/{username}/model_test")
    client.exec_command(f"mkdir -p /home/{username}/model_test/{model}")
    # 上传测试文件
    ...
    _,stdout,_ = client.exec_command(f"python --version", get_pty=True)
    print("hello")
    print(stdout.read().decode())
    #------------step6.从远程服务器下载结果文件------------
    sftp = client.open_sftp()
    result_file_path = os.path.join(code_cache_path, f"{task}_Result.log")
