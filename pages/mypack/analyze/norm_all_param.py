...
def normalize_data(data, reverse=False, use_min_max=False):
    ...
    if use_min_max:
        ...
        # Map data to (-3, 3) then apply sigmoid normalization
        mapped_data = -1 + 1 * (data - data.min(axis=0)) / (data.max(axis=0) - data.min(axis=0))
        if reverse:
...
def process_folder(directory):
    ...
    larger_better = [
        'infer_energy_efficiency.csv', 'infer_throughput.csv', 'train_GPU_Utilization.csv', 'infer_GPU_Utilization.csv',
        'train_energy_efficiency.csv', 'train_throughput.csv', 'train_Memory_Utilization.csv', 'infer_Memory_Utilization.csv'
    ]
